...
public:  
    CSongSearchJob(CGameContext *pGameContext, int ClientID, std::shared_ptr<CHttpRequest> pRequest)  
...
public:
	...
	std::queue<SongInfo> m_PlaylistQueue; 
	...
	void CopyGroupsAndLayers(CDataFileReader &Reader, CDataFileWriter &Writer)  ;
	void CopyExistingMapData(CDataFileReader &Reader, CDataFileWriter &Writer)  ;
	int GetNextLayerIndex();
	void CreateMapSound(int Sound, int Target);
	int m_LastAddedSoundId=1;
