...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("download_song", "s[title] s[artist] s[url]", CFGFLAG_SERVER, ConDownloadSong, this, "Download song after vote");
...
{  
    ...
    str_format(aFilePath, sizeof(aFilePath), "data/musicso/%s.opus", pSongName);  
      
    void *pAudioData;  
    unsigned AudioDataSize;  
    if(!pSelf->Storage()->ReadFile(aFilePath, IStorage::TYPE_ALL, &pAudioData, &AudioDataSize))  
    {  
        char aBuf[256];  
...
{    
    ...
    dbg_msg("song", "Existing sounds in map: %d (starting at index %d)", SoundNum, SoundStart);  
      
    int LayerStart, LayerNum;  
    Reader.GetType(MAPITEMTYPE_LAYER, &LayerStart, &LayerNum);  
    dbg_msg("song", "Existing layers in map: %d (starting at index %d)", LayerNum, LayerStart);  
    ...
    CDataFileWriter Writer;  
    if(!Writer.Open(Storage(), aTempPath, IStorage::TYPE_SAVE))  
    {  
        dbg_msg("song", "ERROR: Failed to create temp map file: %s", aTempPath);  
        Reader.Close();  
        return false;  
    }  
      
    dbg_msg("song", "Successfully created temporary file writer");  
  

	dbg_msg("song", "Starting to copy existing map items...");  
	int CopiedItems = 0;  
	...
	{  
		...
		{  
			dbg_msg("song", "Skipping item type %d at index %d", Type, Index);  
			continue;  
		}  
		...
		if(Type == MAPITEMTYPE_SOUND)  
		{  
			dbg_msg("song", "Copied existing sound item: Type=%d, Id=%d, Size=%d", Type, Id, Size);  
		}  
		else if(Type == MAPITEMTYPE_LAYER)  
		{  
			dbg_msg("song", "Copied existing layer item: Type=%d, Id=%d, Size=%d", Type, Id, Size);  
		}  
	}  
	dbg_msg("song", "Copied %d items from original map (excluding groups)", CopiedItems);
  

    dbg_msg("song", "Starting to copy existing map data...");  
    int CopiedDataBlocks = 0;  
    ...
    {  
        ...
        {  
            dbg_msg("song", "WARNING: Skipping invalid data block at index %d (ptr=%p, size=%d)", Index, pPtr, Size);  
        }  
    }  
    dbg_msg("song", "Copied %d data blocks from original map", CopiedDataBlocks);  
  

    dbg_msg("song", "Adding new audio data...");  
    int SoundDataIndex = Writer.AddData(AudioDataSize, pAudioData);  
    int SoundNameIndex = Writer.AddDataString(pSoundName);  
    dbg_msg("song", "Added audio data at index %d, name at index %d", SoundDataIndex, SoundNameIndex);  
      

    CMapItemSound SoundItem;  
    SoundItem.m_Version = 1;  
    SoundItem.m_External = 0;  
    SoundItem.m_SoundName = SoundNameIndex;  
    SoundItem.m_SoundData = SoundDataIndex;  
    SoundItem.m_SoundDataSize = AudioDataSize;  
      
    dbg_msg("song", "Creating sound item with ID %d", SoundNum);  
    dbg_msg("song", "Sound item details: Version=%d, External=%d, NameIndex=%d, DataIndex=%d, DataSize=%d",   
            SoundItem.m_Version, SoundItem.m_External, SoundItem.m_SoundName, SoundItem.m_SoundData, SoundItem.m_SoundDataSize);  
      
    Writer.AddItem(MAPITEMTYPE_SOUND, SoundNum, sizeof(SoundItem), &SoundItem);  
    dbg_msg("song", "Successfully added sound item with ID %d", SoundNum);  

  
	dbg_msg("song", "Starting dynamic layer index calculation...");  
	
 
	Reader.GetType(MAPITEMTYPE_GROUP, &GroupStart, &GroupNum);  
	dbg_msg("song", "Found %d groups in map", GroupNum);  
	

	CMapItemGroup *pGroup0 = (CMapItemGroup *)Reader.GetItem(GroupStart);  
	if(!pGroup0)  
	{  
		dbg_msg("song", "ERROR: Could not get group 0 information");  
		Reader.Close();  
		Writer.Finish();  
	...
	dbg_msg("song", "Group 0 info: StartLayer=%d, NumLayers=%d", pGroup0->m_StartLayer, pGroup0->m_NumLayers);  
	

	int LayerIndex = -1;
	dbg_msg("song", "Calculated layer index: %d (StartLayer %d + NumLayers %d)",   
			LayerIndex, pGroup0->m_StartLayer, pGroup0->m_NumLayers);  
	
	Reader.GetType(MAPITEMTYPE_GROUP, &GroupStart, &GroupNum);  
	dbg_msg("song", "Found %d groups in map", GroupNum);  
	
	// 获取最后一个组的信息（而不是组0）  
	int LastGroupIndex = GroupNum - 1;  
	CMapItemGroup *pLastGroup = (CMapItemGroup *)Reader.GetItem(GroupStart + LastGroupIndex);  
	if(!pLastGroup)  
	{  
		dbg_msg("song", "ERROR: Could not get last group %d information", LastGroupIndex);  
		Reader.Close();  
		Writer.Finish();  
		Storage()->RemoveFile(aTempPath, IStorage::TYPE_SAVE);  
		return false;  
	...
	dbg_msg("song", "Last group %d info: StartLayer=%d, NumLayers=%d",   
			LastGroupIndex, pLastGroup->m_StartLayer, pLastGroup->m_NumLayers);  
	
	// 计算最后一个组的下一个连续图层索引  
	LayerIndex = pLastGroup->m_StartLayer + pLastGroup->m_NumLayers;  
	dbg_msg("song", "Calculated layer index: %d for last group %d", LayerIndex, LastGroupIndex);  
	
	// 这个索引通常就是总图层数，不会有冲突  
	dbg_msg("song", "Final layer index selected: %d (consecutive to last group)", LayerIndex);

  

    dbg_msg("song", "Creating sound layer for sound index %d", SoundNum);  
    ...
    dbg_msg("song", "Sound source configured:");  
    dbg_msg("song", "  Position: (%d, %d)", Source.m_Position.x, Source.m_Position.y);  
    dbg_msg("song", "  Loop: %d, Pan: %d, TimeDelay: %d, Falloff: %d", Source.m_Loop, Source.m_Pan, Source.m_TimeDelay, Source.m_Falloff);  
    dbg_msg("song", "  Shape: Rectangle %dx%d", Source.m_Shape.m_Rectangle.m_Width, Source.m_Shape.m_Rectangle.m_Height);  
      

    int SourceDataIndex = Writer.AddDataSwapped(sizeof(CSoundSource), &Source);  
    dbg_msg("song", "Added sound source data at index %d", SourceDataIndex);  
    ...
    LayerItem.m_Sound = SoundNum;  
    LayerItem.m_NumSources = 1;  
    LayerItem.m_Data = SourceDataIndex;  
    ...
    dbg_msg("song", "Sound layer configured:");  
    dbg_msg("song", "  Version: %d, Layer.Version: %d, Layer.Type: %d", LayerItem.m_Version, LayerItem.m_Layer.m_Version, LayerItem.m_Layer.m_Type);  
    dbg_msg("song", "  Sound: %d, NumSources: %d, Data: %d", LayerItem.m_Sound, LayerItem.m_NumSources, LayerItem.m_Data);  
    dbg_msg("song", "  Layer name: %s", aLayerName);  
      


    dbg_msg("song", "Adding sound layer with index %d", LayerIndex);  
      
	Writer.AddItem(MAPITEMTYPE_LAYER, LayerIndex, sizeof(LayerItem), &LayerItem);  
	dbg_msg("song", "Successfully added sound layer with index %d", LayerIndex);  
	
 
	dbg_msg("song", "Starting to process all group items...");  
	

	Reader.GetType(MAPITEMTYPE_GROUP, &GroupStart, &GroupNum);  
	dbg_msg("song", "Found %d groups in original map", GroupNum);  
	
	for(int g = 0; g < GroupNum; g++)  
	{  
		CMapItemGroup *pOriginalGroup = (CMapItemGroup *)Reader.GetItem(GroupStart + g);  
		if(!pOriginalGroup)  
		{  
			dbg_msg("song", "WARNING: Could not get group %d", g);  
			continue;  
		}  
		...
		{  
			...
			dbg_msg("song", "Updated last group %d: NumLayers changed from %d to %d",   
					g, pOriginalGroup->m_NumLayers, NewGroup.m_NumLayers);  
		}  
		else  
		{  
			dbg_msg("song", "Keeping group %d unchanged: NumLayers=%d, StartLayer=%d",   
					g, NewGroup.m_NumLayers, NewGroup.m_StartLayer);  
		}  
		
		Writer.AddItem(MAPITEMTYPE_GROUP, g, sizeof(NewGroup), &NewGroup);  
	}
	
	dbg_msg("song", "Successfully processed all group items");
  
    Reader.Close();  
    dbg_msg("song", "Closed map reader");  
...
{
	// 复制版本信息
	const CMapItemVersion *pVersion = (CMapItemVersion *)Reader.FindItem(MAPITEMTYPE_VERSION, 0);
	if(pVersion)
		Writer.AddItem(MAPITEMTYPE_VERSION, 0, sizeof(CMapItemVersion), pVersion);

	// 添加以下完整代码块：  
	// === 开始添加图层组关联代码 ===  
	dbg_msg("song", "Starting to update group information to include new sound layer...");  
	
	// 重新复制所有组，但更新第一个组以包含新的音频图层  
	int GroupStart, GroupNum;  
	Reader.GetType(MAPITEMTYPE_GROUP, &GroupStart, &GroupNum);  
	dbg_msg("song", "Found %d groups in original map", GroupNum);  
	...
	{  
		...
		// 如果是第一个组（通常是默认组），增加图层数量以包含新的音频图层  
		if(g == 0)  
		{  
			NewGroup.m_NumLayers++;  
			dbg_msg("song", "Updated group %d: NumLayers changed from %d to %d", g, pOriginalGroup->m_NumLayers, NewGroup.m_NumLayers);  
			dbg_msg("song", "Group %d details: StartLayer=%d, ParallaxX=%d, ParallaxY=%d", g, NewGroup.m_StartLayer, NewGroup.m_ParallaxX, NewGroup.m_ParallaxY);  
		}  
		else  
		{  
			dbg_msg("song", "Keeping group %d unchanged: NumLayers=%d, StartLayer=%d", g, NewGroup.m_NumLayers, NewGroup.m_StartLayer);  
		}  
		
		// 重新添加组项目（这会覆盖之前复制的组信息）  
		Writer.AddItem(MAPITEMTYPE_GROUP, g, sizeof(NewGroup), &NewGroup);  
	}  
	
	dbg_msg("song", "Successfully updated all group information");  
	// === 结束添加图层组关联代码 === 
	...
	for(int i = 0; i < Num; i++)
	{
		int ItemSize = Reader.GetItemSize(Start + i);
		void *pItem = Reader.GetItem(Start + i);
		if(pItem && ItemSize > 0)
			Writer.AddItem(MAPITEMTYPE_INFO, i, ItemSize, pItem);
	}
	...
	for(int i = 0; i < Num; i++)
	{
		CMapItemImage *pImage = (CMapItemImage *)Reader.GetItem(Start + i);
		if(!pImage)
			continue;

		// 创建图像项目的副本，避免修改原始数据
		CMapItemImage ImageCopy = *pImage;

		if(!pImage->m_External && pImage->m_ImageData >= 0)
		{
			void *pData = Reader.GetData(pImage->m_ImageData);
			int DataSize = Reader.GetDataSize(pImage->m_ImageData);

			// 添加数据验证
			if(pData && DataSize > 0)
			{
				ImageCopy.m_ImageData = Writer.AddData(DataSize, pData);
			}
			else
			{
				Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "Warning: Invalid image data, skipping");
				continue;
			}
		}

		Writer.AddItem(MAPITEMTYPE_IMAGE, i, sizeof(CMapItemImage), &ImageCopy);
	}

	// 复制现有组和图层
	CopyGroupsAndLayers(Reader, Writer);
}

void CGameContext::AddAudioToMap(CDataFileWriter &Writer, CDataFileReader &Reader, const char *pSoundName, void *pAudioData, unsigned AudioDataSize)  
{  
    // 获取现有音频项目数量  
    int Start, Num;  
    ...
    for(int i = 0; i < Num; i++)  
    {  
        CMapItemSound *pExistingSound = (CMapItemSound *)Reader.GetItem(Start + i);  
        if(pExistingSound && pExistingSound->m_SoundName >= 0)  
        {  
            const char *pExistingName = (const char *)Reader.GetData(pExistingSound->m_SoundName);  
            if(str_comp(pExistingName, pSoundName) == 0)  
            {  
                // 音频已存在，不需要重复添加  
                return;  
            }  
    ...
    // 使用下一个可用的ID  
    Writer.AddItem(MAPITEMTYPE_SOUND, Num, sizeof(SoundItem), &SoundItem);  
}

void CGameContext::AddAudioLayer(CDataFileWriter &Writer)  
{  
    // 创建音频源（全地图播放）  
    CSoundSource Source;  
    ...
    Source.m_SoundEnvOffset = 0;  
    Source.m_Shape.m_Type = CSoundShape::SHAPE_RECTANGLE;  
    Source.m_Shape.m_Rectangle.m_Width = 1000000;  // 覆盖整个地图  
    Source.m_Shape.m_Rectangle.m_Height = 1000000;  
    ...
    LayerItem.m_Layer.m_Flags = 0;  
    LayerItem.m_Layer.m_Type = LAYERTYPE_SOUNDS;  
    LayerItem.m_Sound = 0;  // 引用我们刚添加的音频  
    LayerItem.m_NumSources = 1;  
    LayerItem.m_Data = Writer.AddDataSwapped(sizeof(CSoundSource), &Source);  
      
    // 添加图层到地图  
    Writer.AddItem(MAPITEMTYPE_LAYER, GetNextLayerIndex(), sizeof(LayerItem), &LayerItem);  
}

void CGameContext::CopyGroupsAndLayers(CDataFileReader &Reader, CDataFileWriter &Writer)  
{  
    // 复制组  
    int Start, Num;  
    ...
    {  
        int ItemSize = Reader.GetItemSize(Start + i);  
        void *pItem = Reader.GetItem(Start + i);  
        Writer.AddItem(MAPITEMTYPE_GROUP, i, ItemSize, pItem);  
    }  
    ...
    {  
        int ItemSize = Reader.GetItemSize(Start + i);  
        void *pItem = Reader.GetItem(Start + i);  
        Writer.AddItem(MAPITEMTYPE_LAYER, i, ItemSize, pItem);  
    }  
}

int CGameContext::GetNextLayerIndex()  
{  
    // 简单实现：返回一个足够大的索引  
    return 100; // 或者实现更复杂的逻辑来计算实际的下一个索引  
}

void CGameContext::CreateSoundLayer(CDataFileWriter &Writer, int SoundIndex, const char *pSoundName)  
{  
    dbg_msg("song", "Creating sound layer for sound index %d, name: %s", SoundIndex, pSoundName);  
    ...
    dbg_msg("song", "Sound source configured - Position: (%d,%d), Shape: Rectangle %dx%d",   
            Source.m_Position.x, Source.m_Position.y,   
            Source.m_Shape.m_Rectangle.m_Width, Source.m_Shape.m_Rectangle.m_Height);  
    ...
    dbg_msg("song", "Sound layer configured - Version: %d, Sound: %d, NumSources: %d, Data: %d",   
            LayerItem.m_Version, LayerItem.m_Sound, LayerItem.m_NumSources, LayerItem.m_Data);  
	...
	// 计算最大允许长度  
	const int MaxNameLength = std::size(LayerItem.m_aName) * sizeof(int) - 1;  
	
	// 截断过长的名称  
	if(str_length(aLayerName) >= MaxNameLength)  
	{  
		aLayerName[MaxNameLength] = '\\0';  
		dbg_msg("song", "Layer name truncated in CreateSoundLayer: %s", aLayerName);  
	}  
	
	StrToInts(LayerItem.m_aName, std::size(LayerItem.m_aName), aLayerName);
      
    int LayerIndex = GetNextLayerIndex();  
    dbg_msg("song", "Adding sound layer with index %d, name: %s", LayerIndex, aLayerName);  
      
    Writer.AddItem(MAPITEMTYPE_LAYER, LayerIndex, sizeof(LayerItem), &LayerItem);  
    dbg_msg("song", "Sound layer added successfully");  
}

void CGameContext::CreateMapSound(int Sound, int Target)
{
    if(Sound < 0)
        return;
...
{    
    ...
    auto it = pSelf->m_PlayerSongResults.find(ClientID);    
    if(it == pSelf->m_PlayerSongResults.end() || it->second.empty())    
    {    
    ...
    {    
        char aBuf[256];  
        str_format(aBuf, sizeof(aBuf), "无效的编号，请输入 1 到 %d 之间的数字", (int)it->second.size());  
        pSelf->SendChatTarget(ClientID, aBuf);    
        return;    
    }    
        
    const SongInfo &Song = it->second[Index];    
    ...
    // 创建投票命令 - 直接传递歌曲信息而不是索引  
    char aVoteCmd[VOTE_CMD_LENGTH];  
    str_format(aVoteCmd, sizeof(aVoteCmd), "download_song \"%s\" \"%s\" \"%s\"",   
	...
	char aDebugBuf[1024];  
	str_format(aDebugBuf, sizeof(aDebugBuf), "Creating vote command: '%s'", aVoteCmd);  
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", aDebugBuf);  
	
	str_format(aDebugBuf, sizeof(aDebugBuf), "Vote description: '%s'", aVoteDesc);  
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", aDebugBuf);  
	
	pSelf->CallVote(ClientID, aVoteDesc, aVoteCmd, "添加到播放队列", aChatMsg, aVoteDesc);  
	
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "server", "Vote created successfully");
      
    pSelf->CallVote(ClientID, aVoteDesc, aVoteCmd, "添加到播放队列", aChatMsg, aVoteDesc);  
      
    // 清除该玩家的搜索结果  
    pSelf->m_PlayerSongResults.erase(ClientID);  
}
...
{  
    ...
    {  
        m_pGameContext->SendChatTarget(m_ClientID, "搜索失败，请稍后重试");  
        return;  
    }  
    ...
    if(!pJson)  
    {  
        m_pGameContext->SendChatTarget(m_ClientID, "搜索结果解析失败");  
        return;  
    }  
      
    if(pJson->type != json_array)  
    {  
        m_pGameContext->SendChatTarget(m_ClientID, "搜索结果格式错误");  
        json_value_free(pJson);  
        return;  
    }  
      
    // 解析歌曲列表  
    m_pGameContext->m_PlayerSongResults[m_ClientID].clear();  
    std::vector<SongInfo> &Results = m_pGameContext->m_PlayerSongResults[m_ClientID];  
      
    for(unsigned i = 0; i < pJson->u.array.length && i < 10; i++)  
    {  
        json_value *pSong = pJson->u.array.values[i];  
        if(pSong->type != json_object)   
            continue;  
          
        SongInfo Info;  
        for(unsigned j = 0; j < pSong->u.object.length; j++)  
        {  
            const char *pKey = pSong->u.object.values[j].name;  
            json_value *pValue = pSong->u.object.values[j].value;  
              
            if(pValue->type == json_string)  
            {  
                if(str_comp(pKey, "title") == 0)  
                    Info.title = (const char*)pValue->u.string.ptr;  
                else if(str_comp(pKey, "artist") == 0)  
                    Info.artist = (const char*)pValue->u.string.ptr;  
                else if(str_comp(pKey, "page_url") == 0)  
                    Info.page_url = (const char*)pValue->u.string.ptr;  
            }  
        }  
    ...
    m_pGameContext->SendPersonalizedMOTD(m_ClientID, Results);  
    json_value_free(pJson);  
}

void CSongDownloadJob::Run()  
{  
    // 等待HTTP POST请求完成  
    m_pRequest->Wait();  
    ...
    if(m_pRequest->State() != EHttpState::DONE)  
    {  
        m_pGameContext->SendChatTarget(m_ClientID, "下载请求失败，请稍后重试1");  
        return;  
    }  
    ...
    if(!pJson)  
    {  
        m_pGameContext->SendChatTarget(m_ClientID, "下载响应解析失败");  
        return;  
    }  
    ...
    // 清除该玩家的搜索结果  
    m_pGameContext->m_PlayerSongResults.erase(m_ClientID);  
...
{    
    ...
    {    
        ...
        // 修复：考虑歌曲已经播放的时间  
        int64_t Now = time_timestamp();  
        int64_t ElapsedSeconds = Now - m_CurrentSongStartTime;  
        int ElapsedTicks = ElapsedSeconds * Server()->TickSpeed();  
        m_LyricStartTick = Server()->Tick() - ElapsedTicks;  // 调整开始tick  
        m_NextLyricIndex = 0;    
...
{    
    ...
    // 修改：使用歌曲ID而不是队列索引进行保护  
    static std::set<std::string> s_PreloadingSongIds;  
    if(s_PreloadingSongIds.count(pSong->page_url) > 0)  
    {  
        // 减少日志输出频率  
        static int64_t s_LastSkipPrint = 0;  
        int64_t Now = time_timestamp();  
        if(Now - s_LastSkipPrint > 5) // 每5秒最多打印一次  
        {  
    ...
    // 添加到预加载保护集合中  
    s_PreloadingSongIds.insert(pSong->page_url);  
...
{  
	...
	static std::set<std::string> s_PreloadingSongIds;  
	s_PreloadingSongIds.erase(Song.page_url);  
    free(pAudioData);  
}
...
{  
	...
	// 清理所有预加载相关的静态状态  
	static std::set<std::string> s_PreloadingSongIds;  
	s_PreloadingSongIds.clear(); // 切歌时清空所有预加载保护  
	...
	m_CurrentSongId = pNextSong->page_url;  // 更新当前歌曲ID  
	LoadLyrics(m_CurrentSongId);            // 加载新歌曲的歌词  
	if(!m_CurrentLyrics.empty())            // 如果有歌词，启动歌词显示  
	{  
		StartLyrics();  
	}
...
{      
    ...
    // 检查队列中是否有下一首歌  
    if(pSelf->m_PlaylistQueue.size() <= 1)  
    {  
    ...
    // 创建投票命令  
    char aVoteCmd[VOTE_CMD_LENGTH];    
    str_format(aVoteCmd, sizeof(aVoteCmd), "queue_skip");    
